...
NEW_FILES=$(git status --porcelain | grep -oE '\S+-(day|week|book|issue|read|paper|novel)[0-9]+(\.en)?\.html$' | sort -u)
...
N=$((10#$PADDED_N))
KIND=$(echo "$PRIMARY" | grep -oE '(day|week|book|issue|read|paper|novel)')
